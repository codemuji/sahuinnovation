...
<!-- Filters Box -->
<div class="report-filter-toggle" id="filterToggle" onclick="toggleReportFilter()">
    <span><i class="fa fa-filter" style="margin-right: 8px; color: var(--accent);"></i>Filters & Export</span>
    <i class="fa fa-chevron-down chevron" id="filterChevron"></i>
</div>
<div class="report-filter-body desktop-card" id="reportFilterBody" style="margin-bottom: 30px; padding: 20px;">
                    ...
                    <?php else:
                        foreach ($allocations as $a): ?>
                            <tr>
                                ...
                                <td data-label="Allocated By"><?= h($a['admin_name']) ?></td>
                                <td data-label="Amount" style="font-weight: 700; color: var(--accent);"><?= formatCurrency($a['amount']) ?></td>
                                <td data-label="Notes" style="font-size: 12px; max-width: 150px;"><?= h($a['notes']) ?: '-' ?></td>
                            </tr>
                        <?php endforeach;
                    endif; ?>
                </tbody>
                ...
                <tbody>
                    ...
                    <?php else:
                        foreach ($approvedUsages as $u): ?>
                            <tr>
                                <td data-label="Approval Date"><?= date('d M Y', strtotime($u['resolved_at'])) ?></td>
                                <td data-label="Category"><span style="font-size: 12px; font-weight: 600;"><?= h($u['purpose']) ?></span></td>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // On mobile, collapse filters by default if filter was already applied
    if (window.innerWidth <= 767) {
        const applied = <?= json_encode($filterType !== 'year' || $selectedYear != date('Y')) ?>;
        if (!applied) toggleReportFilter(); // start collapsed
    } else {
        // Desktop: always show filter body without toggle chrome
        document.getElementById('filterToggle').style.display = 'none';
        document.getElementById('reportFilterBody').style.display = 'block';
    }
});
