...
// Total Salary & Advance Received
$stmt = $db->prepare("SELECT SUM(amount) FROM salary_disbursements WHERE user_id = ?");
$stmt->execute([$userId]);
...
<div style="display: flex; justify-content: space-between; align-items: center; background: var(--card-bg); border: 1px solid var(--border); padding: 18px 24px; border-radius: var(--radius); margin-bottom: 24px;">
    <div>
        <div style="font-size: 12px; color: var(--text-muted); text-transform: uppercase; font-weight: 700; letter-spacing: 0.5px;">Salary & Advance Disbursed</div>
        <div style="font-size: 24px; font-weight: 800; color: var(--accent); margin-top: 4px;"><?= formatCurrency($totalSalaryAdvance) ?></div>
    </div>
    <a href="salary-advance.php" class="btn btn-outline" style="font-size: 13px; padding: 8px 16px;">View Details &rarr;</a>
</div>
                    ...
                    <?php else:
                        foreach ($allocs as $a): ?>
                            <tr>
                                ...
                                <td data-label="Amount" style="font-weight: 700; color: var(--accent);"><?= formatCurrency($a['amount']) ?></td>
                                <td data-label="Notes" style="font-size: 13px;"><?= h($a['notes']) ?: 'No description' ?></td>
                            </tr>
                        <?php endforeach;
                    endif; ?>
                </tbody>
            ...
            <table class="data-table">
                ...
                <tbody>
                    ...
                    <?php else:
                        foreach ($usages as $u): ?>
                            <tr>
                                <td data-label="Purpose" style="font-weight: 600;"><?= h($u['purpose']) ?></td>
                                <td data-label="Amount" style="font-weight: 700;"><?= formatCurrency($u['amount']) ?></td>
        ...
        <table class="data-table">
            ...
            <tbody>
                ...
                <?php else:
                    foreach ($txs as $t): ?>
                        <tr>
                            <td data-label="Date"><?= date('d M Y H:i', strtotime($t['created_at'])) ?></td>
                            <td data-label="Type">
                                <span style="font-weight: 600; text-transform: uppercase; font-size: 11px; color: <?= $t['type'] === 'credit' ? 'var(--success)' : 'var(--danger)' ?>;">
                                    <?= h($t['type']) ?>
                                </span>
                            </td>
                            <td data-label="Reference"><span style="font-size: 12px; font-weight: 600; text-transform: capitalize; color: var(--text-muted);"><?= str_replace('_', ' ', $t['ref_type']) ?> #<?= $t['ref_id'] ?></span></td>
